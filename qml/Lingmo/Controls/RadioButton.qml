...
T.RadioButton {
    ...
    indicator: Rectangle {
        ...
        color: {
            ...
            if (control.pressed)
                return control.checked ? Qt.darker(Theme.palette.accent, 1.1)
                                      : Qt.lighter(Theme.palette.surface, 1.05)
            if (control.hovered)
                return control.checked ? Qt.lighter(Theme.palette.accent, 1.05)
                                      : Qt.lighter(Theme.palette.surface, 1.02)
            if (control.checked)
        ...
        border {
            width: control.checked ? 0 : (control.visualFocus ? 2 : 1)
            color: {
                if (!control.enabled)
